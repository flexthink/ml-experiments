...
# Cozmo Remote Control with Recording 
# Originally developed by Anki, modified by Artem Ploujnikov to add
# data recording
# Recording is implemented in a naive / non-optimized manner, suitable
...
class RobotStateDisplay(cozmo.annotate.Annotator):
    def apply(self, image, scale):
        ...
        recording_message = (
            'Recording' if remote_control_cozmo and remote_control_cozmo.is_recording
            else 'Not Recording')
        print_line(recording_message)
...
class RobotStateRecorder:
    def __init__(self, rc):
        ...
        self.data = pd.DataFrame(
            columns=[
                ...
                'robot_position_y',
                'robot_angle',
            ] + list(self._default_cube_positions.keys()) + [
                'action_drive_forwards', 
                'action_drive_back',
                'action_turn_left',
                'action_turn_right',
                'action_lift_up',
    ...
    def get_sample(self):
        robot = self.rc.cozmo
        position, rotation = robot.pose.position, robot.pose.rotation
        sample = {
            'robot_position_x': position.x,
            'robot_position_y': position.y,
            'robot_angle': rotation.angle_z.radians,
        ...
        sample.update(self.get_cube_positions())
        return sample
    ...
    def save(self, file_name=None):
        ...
        self.data.to_hdf(file_name, key='robot')
    
    def clear(self):
        self.data.iloc[0:0]

    def generate_file_name(self):
        return time.strftime('robot-%Y%m%d%H%M%S.h5')
...
class RemoteControlCozmo:
    ...
    def use_state_recorder(self, state_recorder):
    ...
    def toggle_recording(self):
        if self.is_recording:
            if self.state_recorder:
                self.state_recorder.save()
                self.state_recorder.clear()
            self.is_recording = False
        else:
            self.is_recording = self.state_recorder is not None
...
def handle_updateCozmo():
    if remote_control_cozmo:
        if state_recorder and remote_control_cozmo.is_recording:
            state_recorder.record_sample()
